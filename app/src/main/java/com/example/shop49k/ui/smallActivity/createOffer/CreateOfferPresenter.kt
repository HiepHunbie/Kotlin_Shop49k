...
class CreateOfferPresenter (postView: CreateOfferView) : BasePresenter<CreateOfferView>(postView){

    @Inject
    lateinit var postApi: PostApi

    private var subscription: Disposable? = null
    fun createOffer(token: String, offerInput: OfferCreateInput, progressBar : ProgressBar, parentActivity: Activity, dialog : Dialog?) {

        progressBar.visibility = View.VISIBLE
        subscription = postApi
            .createOffer(token,offerInput)
            ...
            .subscribe(
                {postList ->
                    if(postList.status == 200){
                        view.successCreateOffer(postList, dialog)
                    }else{
                        view.errorCreateOffer(CommonUtil.convertErrorsToText(postList.errors!!))
                    }
                    progressBar.visibility = View.GONE},
                {
                    ...
                    if (error is HttpException) {
                        ...
                        view.errorCreateOffer(ErrorMessage.getErrorMessage(responseBody,parentActivity))
